# ReplayMac Privacy Policy

**Last updated: 8 July 2026**

ReplayMac does not collect, store, transmit, or share any personal data.

## What happens on your Mac, stays on your Mac

- **Screen and audio recordings** are processed entirely on your Mac and saved only to the output folder you choose (by default `~/Movies/ReplayMac`). They are never uploaded anywhere.
- **Clip metadata** (names, tags, notes, favorites) is stored in a local file inside your output folder.
- **Settings** are stored locally in the app's preferences.

## No collection of any kind

ReplayMac contains no analytics, no telemetry, no crash reporting, no advertising, no user accounts, and no third-party SDKs that phone home.

The Mac App Store version of ReplayMac makes **no network connections at all** — it does not request network access from macOS.

The version distributed on GitHub makes exactly one type of network request: an optional check of the public GitHub Releases page at launch to see whether a newer version exists. This request sends no personal information beyond what any web request inherently includes, and nothing about it is logged or stored by the developer.

## Permissions

ReplayMac asks macOS for Screen Recording (and optionally Microphone) permission solely to create the recordings you ask it to make. You can revoke these at any time in System Settings → Privacy & Security.

## Questions

Open an issue at [github.com/picccassso/ReplayMac](https://github.com/picccassso/ReplayMac/issues).
